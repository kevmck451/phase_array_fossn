...
def high_pass_filter(audio_object, cutoff_freq, order=5):
    ...
    if audio_object.num_channels == 1:
        filtered_data = high_pass(audio_object.data, normal_cutoff, order)

    else:
        for i in range(audio_object.data.shape[0]):
            filtered_data[i, :] = high_pass(audio_object.data[i, :], normal_cutoff, order)
...
if __name__ == '__main__':
    ...
    filtered_data = high_pass_filter(audio, cutoff_frequency)
